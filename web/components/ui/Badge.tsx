...
export default function Badge({ text, variant = "outline" }: BadgeProps) {
  const customVariants = ["test", "odi", "t20", "ipl", "special"];
  const isCustom = customVariants.includes(variant);

  if (isCustom) {
    return (
      <span className={`profile-fbadge profile-fb-${variant} transition-all duration-200 hover:scale-105`}>
        {text}
      </span>
    );
  }
